...
\section{Results}
	\par{
		...
		\ls{
			The average time taken by the \tt{FIFO} scheduler is $3.3$ seconds per
			execution.
		}\ls{
			The average time taken by the \tt{RR} scheduler is $3.1$ seconds per
			execution.
		}\ls{
			The average time taken by the \tt{NORMAL} scheduler is $3.3$ seconds per
			execution.
		}\ls{
			The avreage CPU Utilization of all three schedulers where the same. This KPI
			was particularly difficult to measure as the CPU Utilization was being affected
			by other processes running on the system. 
			\ls{
				This could also be attributed to the fact that the threads were all executing
				the same function, even though the fibonacci function grew exponentially with
				respect to the thread id but it did not affect the CPU Utilization \u{noticeably}.
